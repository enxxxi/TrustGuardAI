...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  List<Transaction> _filtered(List<Transaction> transactions) => transactions.where((tx) {
  ...
  Widget build(BuildContext context) {
    ...
    final list = _filtered(state.transactions);
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${list.length} records - RM ${total.toStringAsFixed(0)} total',
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: Wrap(
              ...
              children: [
                _SChip(
                  '${state.transactions.where((t) => t.status == TxStatus.approved).length}',
                  'Approved',
                  AppColors.safe,
                ),
                _SChip(
                  '${state.transactions.where((t) => t.status == TxStatus.flagged).length}',
                  'Flagged',
                  AppColors.warn,
                ),
                _SChip(
                  '${state.transactions.where((t) => t.status == TxStatus.blocked).length}',
                  'Blocked',
                  AppColors.danger,
                ),
              ],
      ...
      child: AppCard(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                _M('Location', tx.location),
                const Spacer(),
                _M('Time', '${tx.date} - ${tx.time}'),
                const Spacer(),
...
Widget _M(String label, String text) => Row(
      mainAxisSize: MainAxisSize.min,
      children: [
