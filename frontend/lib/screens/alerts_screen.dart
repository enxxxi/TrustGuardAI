  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(
                ...
                style: AppText.label(12,
                  color: unread > 0 ? AppColors.danger : AppColors.safe),
              ),
            ]),
      ...
      Padding(
        ...
        child: Row(children: [
          _ACount(
            state.alerts.where((a) => a.severity == AlertSeverity.danger).length,
            'Critical', AppColors.danger),
          const SizedBox(width: 8),
          _ACount(
            state.alerts.where((a) => a.severity == AlertSeverity.warning).length,
            'Warning', AppColors.warn),
          const SizedBox(width: 8),
          _ACount(
            state.alerts.where((a) => a.severity == AlertSeverity.info).length,
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: unread ? fg.withOpacity(0.3) : AppColors.border,
              width: unread ? 1.5 : 1,
            ),
              ...
              Padding(
                ...
                child: Row(children: [
                  if (widget.alert.severity != AlertSeverity.info) ...[
                    _AChip('Investigate', Icons.search_rounded,
                      AppColors.accent, AppColors.accentLight),
                    const SizedBox(width: 8),
                    _AChip('Dismiss', Icons.check_rounded,
                      AppColors.safe, AppColors.safeLight),
                  ] else
                    _AChip('Mark as read', Icons.visibility_rounded,
                      AppColors.ink2, AppColors.card2),
                ]),
              ),
            ],
          ]),
